<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                <li class="addr-new" @click="addNewAddressConfirm">
          ...
          <div class="next-btn-wrap">
            <a class="btn btn--m btn--red "  v-bind:class="{'btn--dis':addressList.length == 0}" @click="next">Next
            </a>
          </div>
        </div>
      </div>
    ...
    <modal :mdShow="addNew" @close="closeModal">
    <div class="info" slot="message">
      <label>收件人姓名：</label><input type="text" v-model="userName"><br>
      <label>详细地址：</label><input type="text" v-model="streetName"><br>
      <label>邮编：</label><input type="text" v-model="postCode"><br>
      <label>电话：</label><input type="text" v-model="tel">
    </div>
    <div slot="btnGroup">
      <a class="leftBtn btn btn--m" href="javascript:;" @click="addNewAddress">确认</a>
      <a class="rightBtn btn btn--m" href="javascript:;" @click="addNew=false">关闭</a>
    </div>
    </modal>
    <modal :mdShow="addNewInfo" @close="closeModal">
      <p slot="message">{{addNewMsg}}</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="addNewInfo=false">关闭</a>
      </div>
    </modal>
      ...
      addNewAddress(){
        axios.post('/users/addNewAddress',{
          ...
          if(res.status == 0){
            ...
            this.addNewMsg = '添加地址成功'
            this.init();
          }else{
            this.addNewInfo = true;
            this.addNewMsg = '添加地址失败'
          }
        })
      }
    }
