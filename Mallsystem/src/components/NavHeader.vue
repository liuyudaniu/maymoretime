<template>
  <header class="header">
          ...
          <div class="md-modal-inner">
            ...
            <div class="md-content">
              ...
              <div class="login-wrap">
                ...
                <a href="javascript:;" class="register" @click="registerConfirm">免费注册</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal class="info" v-bind:md-show="modalRegister" v-on:close="closeModal">
      <div slot="message">
        <p>Register</p>
        <label>用户名：</label><input type="text" v-model="regName"><br/>
        <label>密码：</label><input type="text" v-model="regPwd">
      </div>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="register">注册</a>
        <a class="btn btn--m" @click="modalRegister = false" href="javascript:;">关闭</a>
      </div>
    </modal>
    <modal v-bind:md-show="modalRegisterConfirm" v-on:close="closeModal">
      <p slot="message">{{registerInfo}}</p>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="loginAgain" v-show="loginInfo">去登录</a>
        <a class="btn btn--m" @click="modalRegisterConfirm = false" href="javascript:;">关闭</a>
      </div>
    </modal>
  </header>
</template>
      ...
      methods:{
        ...
        register(){
          if(this.regName == '' || this.regPwd == ''){
            this.modalRegister = false;
            this.modalRegisterConfirm = true;
            this.registerInfo = '用户名或密码不能为空';
          }else{
        ...
        loginAgain(){
          ...
          this.modalRegisterConfirm = false
        },
        toCart(){
          if(this.nickName != ""){
...
<style scoped>
  ...
  .info label{display:inline-block;width:85px;text-align:right}
